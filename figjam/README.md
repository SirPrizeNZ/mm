...
The FigJam widget is the shared lobby and results card. Clicking **Join race** opens the existing Scale Miniatures game in a local Figma modal for that person. Each client runs the deterministic TypeScript engine and follows its own car. The Node WebSocket relay forwards input packets; FigJam synced state holds the room, lobby names, selected track and laps, and final standings, never live car positions. Each person's FigJam name appears beside their boat and in the results.
...
The 26 September build uses a new development widget ID because the earlier board widget pointed to a deleted GitHub commit after the repository was recreated. Remove that old board instance and import this manifest again; re-rendering the old instance cannot repair its missing code URL. The current FigJam test board has one fresh `MicroMachine` instance.
...
The 24 map thumbnails under `figjam/map-previews/` are generated from the existing game decoders with `node figjam/generate-map-previews.mjs`. They are served as separate PNGs at `/api/maps/{round}-{track}.png` by the same Render service. The widget bundle only contains their URLs; the images have the separate original-game data notice in [`GAME_DATA_NOTICE.md`](../GAME_DATA_NOTICE.md).
