...
export function relayUrl(): string {
  const figjam = (globalThis as typeof globalThis & { __SM_FIGJAM__?: { relayUrl: string } }).__SM_FIGJAM__;
  if (figjam?.relayUrl) return figjam.relayUrl;
  const configured = (import.meta as { env?: { VITE_RELAY_URL?: string } }).env?.VITE_RELAY_URL;
...
export class Relay implements NetTransport {
   ...
   */
  static join(room?: string, url = relayUrl(), createIfMissing = false): Promise<Relay> {
    return new Promise<Relay>((resolve, reject) => {
      ...
      ws.onopen = () => { ws.send(JSON.stringify({ join: room ?? null, createIfMissing })); };
  ...
  publishResult(gen: number, points: readonly number[], names: readonly string[]): void {
    if (this.open) this.ws.send(JSON.stringify({ result: { gen, points, names } }));
  }
