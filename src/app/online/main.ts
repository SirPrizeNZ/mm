...
import { fromServerIfServed, fromFolderInput, whatIsMissing } from '../../hal/fs/openGameFiles';
...
/** Round 7 can dispatch DOS terrain handler 66b2, which the existing engine has not transliterated. */
export function supportsExtendedTrack(leg: Leg): boolean { return leg.round !== 7; }
...
async function openFiles(): Promise<GameFiles> {
  const figjam = (globalThis as typeof globalThis & {
    __SM_FIGJAM__?: { gameDataRoot?: string };
  }).__SM_FIGJAM__;
  const embedded = Boolean(figjam);
  const served = embedded ? undefined : await fromServerIfServed();
  ...
  if (figjam?.gameDataRoot) {
    try {
      ...
      if (whatIsMissing(remote) === undefined && await remote.served()) return remote;
    } catch { /* The folder picker below remains available if this source is unavailable. */ }
  }
  const box = $('#needfiles');
...
function runLeg(wire: NetTransport, files: RaceFiles, deal: Deal, mine: number, gen: number,
                names: ReadonlyMap<number, string>, sound?: SoundOutput,
                departed: ReadonlySet<number> = new Set()): Promise<Result | undefined> {
  lobby.hidden = true;
  stage.hidden = false;
  under.hidden = false;
  board.hidden = true;
  ...
  if (own) { own.jumpTo(built.ds, built.ds.cars[mine]!); renderer.view = own; renderer.hudCar = built.ds.cars[mine]!; }
  ...
  const present = (): void => {
    ...
    if (own) for (let i = 0; i < deal.humans; i++) {
      ...
      if (built.ds.r16(bx + 0x124C) === 0) continue;
      let x = (built.ds.r16(bx + 0x125C) - own.x + 0xC00) % 0xC00;
      let y = (built.ds.r16(bx + 0x1268) - own.y + 0xC00) % 0xC00;
      if (x >= VP.outWidth || y >= VP.height) continue;
      const name = names.get(i) ?? 'Guest';
      ctx.font = 'bold 8px Arial';
      ctx.textAlign = i & 1 ? 'right' : 'left';
      ctx.lineWidth = 2;
      ctx.strokeStyle = '#000'; ctx.fillStyle = i === mine ? '#ffe800' : '#fff';
      const labelX = x + (i & 1 ? -10 : 10);
      ctx.strokeText(name, labelX, y - 5, 72); ctx.fillText(name, labelX, y - 5, 72);
    }
  };
   ...
   */
  const left = new Set<number>(departed);
  ...
  return new Promise<Result | undefined>(done => {
    ...
    const tick = (now: number): void => {
      ...
      if (peer.over) {
        ...
        present();
        ended = true;
        finish({ places: built.ds.cars.slice(0, deal.cars).map(bx => built.ds.r16(bx + 0x12EF)) });
        return;
      }
    };
...
function showBoard(deal: Deal, table: number[], gen: number, mine: number,
                   names: ReadonlyMap<number, string>): void {
  stage.hidden = true;
  under.hidden = true;
  board.hidden = false;
...
async function main(): Promise<void> {
  ...
  const code = $('#code');
  const who = $('#who');
  const players = $<HTMLOListElement>('#players');
  ...
  if (!figjam) startHero($('#head'), $<HTMLCanvasElement>('#hero-canvas'), `${base}mm-boat.png`, `${base}mm-duck.png`);
  ...
  const enter = async (want?: string, boardSession = false): Promise<void> => {
    say('connecting…');
    let wire: Relay;
    try { wire = await Relay.join(want, undefined, boardSession); }
    ...
    const names = new Map<number, string>([[wire.slot, myName]]);
    ...
    wire.onLeave = slot => {
      if (started) departed.add(slot);
      loaded.delete(slot);
      room();
      if (!started) say('somebody left the room');
    ...
    /** Everybody has the same files before this starts, so the flag cannot drop while a client is loading. */
    const countDown = async (): Promise<void> => {
      ...
      await new Promise<void>(resolve => {
        const draw = (): void => {
          ...
          if (left <= 0) { clearInterval(timer); countdown.hidden = true; resolve(); }
    ...
    const play = async (deal: Deal): Promise<void> => {
      ...
      for (let gen = 0; gen < deal.legs.length; gen++) {
        ...
        if (boardSession) {
          const points = table.slice(0, deal.cars);
          wire.publishResult(gen, points, Array.from({ length: deal.cars }, (_, i) => names.get(i) ?? 'Computer'));
          if (figjam) parent.postMessage({ pluginMessage: {
            type: 'result', session: figjam.session, result: {
              gen, points, names: Array.from({ length: deal.cars }, (_, i) => names.get(i) ?? 'Computer'), at: Date.now(),
            },
          } }, '*');
        }
        if (gen + 1 < deal.legs.length) await new Promise(res => setTimeout(res, 4000));
      }
      say(boardSession ? 'Race complete. Results are on the FigJam board.'
        : 'that is the lot. Reload to play again.');
    };
  ...
  else if (invited) { join.value = invited.toUpperCase(); void enter(invited.toUpperCase()); }
  else say('make a room, or type the code somebody gave you.');
}

export function startOnline(): void {
  void main().catch((e: unknown) => { say(String(e)); });
}

if (!(globalThis as typeof globalThis & { __SM_FIGJAM_WAIT__?: boolean }).__SM_FIGJAM_WAIT__) startOnline();
