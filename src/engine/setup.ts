...
import { applyExtendedCarPalette, assertTintPaletteSlotsUnused, extendedCarTint } from './car-colours';
...
export interface RaceParams {
  ...
  /**
   * How many cars are in the race at all, 2 to 10. Fewer than four is not an invention: head to head runs
  ...
  /**
   * How many cars a person is driving.
   *
...
/** The four DOS car records alias other segment data if simply continued. Extra online records are copied
 * into the extended portion of DataSegment after all original file loads, then placed on rows behind the
 * original grid. The same per-car physics reads these records by their full JavaScript offsets. */
function initExtraCars(d: DataSegment, count: number, humans: number): void {
  const first = 0x124A;
  // Preserve the first four DOS colour offsets. Extended cars carry a marker and car index so the renderer
  // uses the dedicated orange, cyan, violet, white, black and grey palette entries.
  // DOS resets every start heading to zero (north). Its rear grid row is cars 0 and 1 at y + 0x1A;
  // continue that two-column pattern south of those cars, rather than risking overlap with the front row.
  const wrap = (v: number): number => (v + 0xC00) % 0xC00;
  for (let i = 4; i < count; i++) {
    const bx = d.cars[i]!;
    const template = CARS[i % 4]! + first;
    d.m.set(d.m.subarray(template, template + 0x164), bx + first);
    const row = Math.floor((i - 4) / 2) + 1;
    const source = CARS[i & 1]!;
    const x = d.r16(source + 0x125C);
    const y = wrap(d.r16(source + 0x1268) + row * 0x1A);
    for (const o of [0x125C, 0x125E, 0x12F1]) d.w16(bx + o, x);
    for (const o of [0x1268, 0x126A, 0x12F3]) d.w16(bx + o, y);
    d.w16(bx + 0x124A, i + 1);
    d.w16(bx + 0x124C, 1); d.w16(bx + 0x124E, 1);
    d.w16(bx + 0x1252, extendedCarTint(i));
    d.w16(bx + 0x12EB, i >= humans ? 1 : 0);
    d.w16(bx + 0x12EF, i + 1);
    d.w16(bx + 0x137C, (i & 1) | ((row & 1) << 1));
  }
}
