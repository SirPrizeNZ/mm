...
import { recolourCarPixel } from './car-colours';
...
export class RaceRenderer {
  ...
  private spritePass(): void {
    ...
    for (const bx of d.cars) if (d.r16(bx + 0x124C) !== 0 && d.r16(bx + 0x12D6) !== 0) this.shadow(bx, round);
    // 2. skid marks / tyre tracks (drawing halves of fn 8386 / 8083); images live in BITSFILE.PH0 at ds:3fe3
    for (const bx of d.cars) { this.skidMarks(bx); this.tyreTracks(bx); }
    // 3. particles (fn 8712 drawing): splash / thrown-up bits with a shadow while high
    for (const bx of d.cars) if (d.r16(bx + 0x13A4) !== 0) this.particle(bx);
  ...
  private rankMarker(bx: number): void {
    const d = this.src.ds;
    const rank = d.r16(bx + 0x12EF);
    if (rank < 1 || rank > 4) return;   // DOS only contains four position marker sprites
    const src = 0x5CE3 + ((rank - 1) << 7);
  ...
  private pulledAnim(bx: number, round: number): void {
    ...
    if (round === 9) {
      ...
      if (c) this.blitSprite(frame * 0x240 + c.srcOff, c.di, c.w, c.h, c.skip, 'recolour', d.r16(bx + 0x1252), extra);
    }
  }
  ...
  private tableAnim(bx: number, round: number, table: number): void {
    ...
    const c = this.clip(s16(x - 0xC), s16(y - 0xC), 0x18, 0x18);
    if (c) this.blitSprite(frame * 0x240 + c.srcOff, c.di, c.w, c.h, c.skip, 'recolour', d.r16(bx + 0x1252), this.src.extra);
  }
  ...
  private hud(): void {
    ...
    const order = d.cars.length > 4
      ? [...d.cars].sort((a, b) => d.r16(a + 0x12EF) - d.r16(b + 0x12EF))
      : CARS.map((_, i) => d.r16(0x2678 + i * 2));
  ...
  private carIcon(bx: number, di: number): void {
    const m = this.src.ds.m;
    const src = this.src.ds.r16(bx + 0x12ED) !== 0 ? 0x5363 : 0x5BE3, colour = this.src.ds.r16(bx + 0x1252);
  ...
  private car(bx: number, round: number, at?: { x: number; y: number; dz: number }): void {
    ...
    if (!c) return;
    const colour = round === 9 ? 0 : d.r16(bx + 0x1252);
  ...
  private blitSprite(src: number, di: number, w: number, h: number, skip: number, mode: 'recolour' | 'plain' | 'shadow', colour: number,
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        ...
        if (px !== 0) {
          if (mode === 'shadow') px = 0;
          else if (mode === 'recolour' && (px & 0xF) <= 2) px = recolourCarPixel(px, colour);
