...
export function pollInput(d: DataSegment, ai?: (bx: number) => number, dev?: AnalogueDevices,
  ...
  d.cars.forEach((bx, i) => {
    const src = i < 4 ? d.r16(0x2658 + i * 2) : 6;
...
export class Race {
  ...
  private engineSound(): void {
    ...
    for (const bx of d.cars) if (d.r16(bx + 0x12AE) === 0x0B) return;
    const cars = d.r16(0x2656) === 2 ? 2 : d.cars.length;
  ...
  private carControl(bx: number): void {
    ...
    if (d.r16(0x2656) !== 2) {
      ...
      for (const car of d.cars)
        if (d.r16(car + 0x12ED) === 0 && d.r16(car + 0x127A) === 0) d.add16(0x26C6, 1);
      if (d.cars.length > 4
          ? d.cars.some(car => d.r16(car + 0x12ED) === 0)
          : d.r16(d.r16(0x2660) + 0x12ED) === 0) d.w16(0x26C6, 2);
      return;
    }
  ...
  private carControlMain(bx: number): void {              // 4d04
    ...
    if (d.r16(bx + 0x12EB) !== 1) {
      const ax = bx >= 0x10000 ? 4 : d.r16(0x2658 + (d.r16(bx + 0x124A) - 1) * 2);
  ...
  terrainHandler(bx: number, addr: number): void {
    switch (addr) {
      ...
      default: throw new NotImplementedYet(`terrain handler ${addr.toString(16)} for car ${this.d.cars.indexOf(bx) + 1}`);
    }
  }
  ...
  fn5960Pair(bxEntry: number, pairA?: number, pairB?: number): number {
    const d = this.d;
    this.candidate(bxEntry);                                // quirk: recomputes the candidate of whatever bx holds
    const A = pairA ?? d.r16(0x27B1), B = pairB ?? d.r16(0x27B3);
  ...
  private camera(): void {
    ...
    for (const car of d.cars) d.w16(car + 0x137E, 0);
    ...
    if (d.r16(0x2650) === 0x32 && d.r16(0x264E) === 0x32) for (const car of d.cars) d.w16(car + 0x137E, 1);
  }
  ...
  private lapBase(): number {
    let n = 9;
    for (const bx of this.d.cars) n = Math.max(n, this.d.rs16(bx + 0x12ED));
    return n;
  }
  ...
  fn8dfcRanking(): void {
    ...
    if (d.cars.length > 4) {
      const cl = d.r16(0x2652) & 0xFF, base = this.lapBase();
      const score = (bx: number): number => d.rs16(bx + 0x12ED) > 0 && d.rs16(0x26C6) < 2
        ? ((base - d.r16(bx + 0x12ED)) & 0xFF) * cl + d.r16(bx + 0x12E3) : 0x7D00;
      const order = [...d.cars].sort((a, b) => score(b) - score(a) || d.cars.indexOf(a) - d.cars.indexOf(b));
      order.forEach((bx, i) => d.w16(bx + 0x12EF, i + 1));
      return;
    }
  ...
  renderSideEffects(): void {
    ...
    for (const bx of d.cars) { this.fn8386Skids(bx); this.fn8083Tracks(bx); }
    for (const bx of d.cars) if (d.r16(bx + 0x13A4) !== 0) this.fn8712Particles(bx);
    for (const bx of d.cars) {
  ...
  fn6ae5Pocket(bx: number): void {
    const d = this.d;
    // The DOS pocket queue stores 16-bit car pointers. Virtual cars cannot be placed in it, so they take
    // the existing shortcut/respawn path instead of aliasing car 0's queue entry and corrupting the race.
    if (bx >= 0x10000) return this.shortcut5f85(bx);
  ...
  fn849bCountdown(bx: number): void {
    ...
    if (d.rs16(0x26D5) >= 0x60) {
      for (const car of d.cars) d.w16(car + 0x12AE, 0);
