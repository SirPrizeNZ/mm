...
describe('two clients and a relay', () => {
  ...
  it('gives four board players separate slots, forwards only their bytes, and returns standings', async () => {
    ...
    for (let slot = 0; slot < 4; slot++) {
      ...
      clients.push(client);
      client.ws.send(JSON.stringify({ join: session, createIfMissing: true }));
      await settle();
      expect(JSON.parse(client.text[0]!)).toMatchObject({ room: session, slot });
    }
    for (let slot = 0; slot < 4; slot++) clients[slot]!.ws.send(new Uint8Array([1, slot + 10]));
    await settle();
    for (let slot = 0; slot < 4; slot++) {
      expect(clients[slot]!.binary).toHaveLength(3);
      expect(clients[slot]!.binary).toEqual(expect.arrayContaining(
        [0, 1, 2, 3].filter(other => other !== slot).map(other => [other, 1, other + 10])));
    }
    clients[3]!.ws.send(JSON.stringify({ result: { gen: 0, points: [4, 3, 2, 1], names: ['A', 'B', 'C', 'D'] } }));
    await settle();
    const response = await fetch(`http://127.0.0.1:${port}/api/relay/result?session=${session}`);
    expect(response.status).toBe(200);
    expect(await response.json()).toMatchObject({ gen: 0, points: [4, 3, 2, 1], names: ['A', 'B', 'C', 'D'] });
    const fifth = await open();
    fifth.ws.send(JSON.stringify({ join: session, createIfMissing: true }));
    await settle();
    expect(JSON.parse(fifth.text[0]!)).toMatchObject({ room: session, slot: 4 });
    fifth.ws.close();
