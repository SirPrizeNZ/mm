...
describe.skipIf(!have)('extended online cars', () => {
  it('ends a ten-car race when an extra car finishes', () => {
    const race = new Race(setupRace(files(2, 1), { ...FOUR, cars: 10, humanCars: 10 }).ds);
    race.followLeader = true; race.catchup = false;
    const sim = new Sim(race, () => 0, 2);
    const extra = race.d.cars[9]!;
    race.d.w16(extra + 0x12ED, 0);
    race.d.w16(extra + 0x127A, 0);
    sim.advanceTo(1);
    expect(race.d.r16(0x26C6)).toBe(2);
    sim.advanceTo(250);
    expect(sim.over).toBe(true);
  });
  it('places six extra cars on drivable cells across the race tracks', () => {
    const blocked: string[] = [];
    for (let round = 1; round <= 8; round++) for (let track = 1; track <= 3; track++) {
      const race = new Race(setupRace(files(round, track), {
        ...FOUR, round, track, cars: 10, humanCars: 10,
      }).ds);
      for (let i = 4; i < 10; i++) {
        const bx = race.d.cars[i]!;
        if (race.fn589cCol(race.d.r16(bx + 0x125C), race.d.r16(bx + 0x1268)).solid)
          blocked.push(`${round}.${track}:${i + 1}`);
      }
    }
    expect(blocked).toEqual([]);
  });
  it('advances ten cars across every supported extended race track', () => {
    const failures: string[] = [];
    for (let round = 1; round <= 8; round++) for (let track = 1; track <= 3; track++) {
      if (round === 7) continue; // the original engine has no implementation for terrain handler 66b2
      const race = new Race(setupRace(files(round, track), {
        ...FOUR, round, track, cars: 10, humanCars: 10,
      }).ds);
      race.followLeader = true; race.catchup = false;
      const sim = new Sim(race, (car, step) => 0x20 | ((step + car * 17) % 113 < 20 ? 0x80 : 0), 2);
      try { sim.advanceTo(2000); }
      catch (e) { failures.push(`round ${round} track ${track} step ${sim.step}: ${String(e)}`); }
    }
    expect(failures).toEqual([]);
  });
  for (const count of [5, 6, 8, 10]) it(`${count} cars have independent input and identical state on two machines`, () => {
    const make = (lastInput: number): Sim => {
      const race = new Race(setupRace(files(2, 1), { ...FOUR, cars: count, humanCars: count }).ds, DOS_VIEWPORT);
      race.followLeader = true; race.catchup = false;
      return new Sim(race, car => car === count - 1 ? lastInput : 0, 2);
    };
    const still = make(0), drive = make(0x20), mirror = make(0x20);
    const last = drive.d.cars[count - 1]!;
    expect(new Set(drive.d.cars).size).toBe(count);
    expect(drive.d.r16(last + 0x124C)).toBe(1);
    still.advanceTo(200); drive.advanceTo(200); mirror.advanceTo(200);
    expect(drive.hash()).toBe(mirror.hash());
    expect(drive.d.r16(last + 0x125C) !== still.d.r16(last + 0x125C)
      || drive.d.r16(last + 0x1268) !== still.d.r16(last + 0x1268)).toBe(true);
    expect(drive.hash()).not.toBe(still.hash());
  });
});
