...
/** FigJam widgets use a longer shared session key to avoid collisions across boards. */
export const SESSION_LENGTH = 20;
/** Room slots: up to ten cars, ten people. */
...
export class Rooms {
  ...
  /** A board session is created by the first browser to arrive, without a separate host handshake. */
  ensure(code) {
    this.sweep();
    if (this.rooms.has(code)) return true;
    if (this.rooms.size >= this.max) return false;
    this.rooms.set(code, { members: new Map(), at: this.now() });
    return true;
  }
  ...
  leave(code, slot) {
    ...
    if (room.members.size === 0 && !room.result) this.rooms.delete(code);
    else room.at = this.now();
  }
  ...
  result(code) { return this.rooms.get(code)?.result; }

  recordResult(code, result) {
    const room = this.rooms.get(code);
    if (!room) return;
    if (room.result && result.gen <= room.result.gen) return;
    room.result = result;
    room.at = this.now();
  }
...
export function parseJoin(text) {
  ...
  if (body.createIfMissing === true && typeof body.join === 'string'
      && new RegExp(`^[A-Z2-9]{${SESSION_LENGTH}}$`).test(body.join)) {
    return { code: body.join, ensure: true };
  }
...
export function start(port = PORT) {
  ...
  const server = createServer((req, res) => {
    if ((req.url ?? '').split('?')[0] === '/api/relay/result') {
      ...
      rooms.sweep();
      const result = rooms.result(code);
      res.writeHead(result ? 200 : 404, {
        'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store',
        'Access-Control-Allow-Origin': '*',
      });
      return res.end(JSON.stringify(result ?? { error: 'no result yet' }));
    }
    ...
    socket.on('data', chunk => {
      ...
      for (const { opcode, payload } of messages) {
        ...
        if (client.code === undefined) {                       // still on the doorstep
          ...
          if (want.ensure && !rooms.ensure(code)) {
            send(OP.TEXT, JSON.stringify({ error: 'no room for another room' })); continue;
          }
        ...
        // A final standings note is the sole metadata the FigJam board reads. Car inputs remain opaque.
        if (opcode === OP.TEXT) {
          let note;
          try { note = JSON.parse(payload.toString('utf8')); } catch { note = null; }
          const result = note?.result;
          if (result && Number.isInteger(result.gen) && result.gen >= 0 && result.gen < 12
              && Array.isArray(result.points) && result.points.length >= 2 && result.points.length <= MAX_PLAYERS
              && result.points.every(n => Number.isInteger(n) && n >= 0 && n <= 1000)
              && Array.isArray(result.names) && result.names.length === result.points.length
              && result.names.every(n => typeof n === 'string' && n.length <= 40)) {
            rooms.recordResult(client.code, { gen: result.gen, points: result.points, names: result.names, at: Date.now() });
            continue;
          }
        }
